...
void ZSubtractSWCsDialog::init()
{
  ...
  m_subtractSWCsWidget = new ZSelectFileWidget(ZSelectFileWidget::OPEN_MULTIPLE_FILES, "Subtract SWCs:", tr("SWCs (*.swc)"));
  alllayout->addWidget(m_subtractSWCsWidget);
  m_inputSWCWidget = new ZSelectFileWidget(ZSelectFileWidget::OPEN_SINGLE_FILE, "From SWC:", tr("SWCs (*.swc)"));
  alllayout->addWidget(m_inputSWCWidget);
  alllayout->addSpacing(5);
  m_outputSWCWidget = new ZSelectFileWidget(ZSelectFileWidget::SAVE_FILE, "Output SWC:", tr("SWC (*.swc)"));
...
void ZSubtractSWCsDialog::subtractSWCs()
{
  ...
  QMessageBox::information(this, "Done", "Subtract SWCs Finished.");
}
