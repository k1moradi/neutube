...
public:
  ...
  inline ZFlyEmBodyMergeFrame* getDataFrame() { return m_dataFrame; }
  // Obsolete functions
  ...
  T* getDocument() const;
  void syncWithDvid();
  void setSelection(const std::set<uint64_t>& selected, NeuTube::EBodyLabelType labelType);
...
public slots:
  ...
  void update3DBodyView(bool showingWindow = true, bool resettingCamera = false);
...
private:
  ...
  void update3DBodyViewPlane(const ZDvidInfo& dvidInfo, const ZStackViewParam& viewParam);
  ...
  void mergeBodyAnnotation(uint64_t targetId, const std::vector<uint64_t>& bodyId);
private:
