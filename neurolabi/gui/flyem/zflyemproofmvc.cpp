...
void ZFlyEmProofMvc::initBodyWindow() {
  ...
  m_bodyWindowFactory = QSharedPointer<ZWindowFactory>(new ZFlyEmBodyWindowFactory);
  ...
  m_bodyViewWindow->m_stayOnTopAction = m_bodyViewWindow->toolBar->addAction("Pin");
  m_bodyViewWindow->m_stayOnTopAction->setCheckable(true);
  m_bodyViewWindow->m_stayOnTopAction->setChecked(false);
  connect(m_bodyViewWindow->m_stayOnTopAction, SIGNAL(triggered(bool)), m_bodyViewWindow, SLOT(stayOnTop(bool)));
...
ZFlyEmProofMvc* ZFlyEmProofMvc::Make(const ZDvidTarget& target) {
  ...
  ZFlyEmProofMvc* mvc = ZFlyEmProofMvc::Make(NULL, ZSharedPointer<ZFlyEmProofDoc>(doc));
  mvc->getPresenter()->setObjectStyle(ZStackObject::SOLID);
  mvc->setDvidTarget(target);
  connect(mvc->getPresenter(), SIGNAL(orthoViewTriggered(double, double, double)), mvc, SLOT(showOrthoWindow(double, double, double)));
  return mvc;
}
...
void ZFlyEmProofMvc::setWindowSignalSlot(Z3DWindow* window) {
  if(window != NULL) {
    if(window == m_coarseBodyWindow) {
      connect(window, SIGNAL(destroyed()), this, SLOT(detachCoarseBodyWindow()));
      connect(window, SIGNAL(croppingSwcInRoi()), this, SLOT(cropCoarseBody3D()));
    ...
    connect(window, SIGNAL(locating2DViewTriggered(ZStackViewParam)), this->getView(), SLOT(setView(ZStackViewParam)));
  }
}
...
void ZFlyEmProofMvc::makeOrthoWindow() {
  ...
  connect(m_orthoWindow, SIGNAL(bookmarkEdited(int, int, int)), getCompleteDocument(), SLOT(downloadBookmark(int, int, int)));
  connect(getCompleteDocument(), SIGNAL(bookmarkEdited(int, int, int)), m_orthoWindow, SLOT(downloadBookmark(int, int, int)));
  connect(m_orthoWindow, SIGNAL(synapseEdited(int, int, int)), getCompleteDocument(), SLOT(downloadSynapse(int, int, int)));
  connect(getCompleteDocument(), SIGNAL(synapseEdited(int, int, int)), m_orthoWindow, SLOT(downloadSynapse(int, int, int)));
  connect(m_orthoWindow, SIGNAL(zoomingTo(int, int, int)), this, SLOT(zoomTo(int, int, int)));
  connect(m_orthoWindow, SIGNAL(bodyMergeEdited()), this, SLOT(syncMergeWithDvid()));
  connect(this, SIGNAL(bodyMergeEdited()), m_orthoWindow, SLOT(syncMergeWithDvid()));
...
void ZFlyEmProofMvc::makeCoarseBodyWindow() {
  ...
  connect(m_coarseBodyWindow->getPunctaFilter(), SIGNAL(visibleChanged(bool)), doc, SLOT(showSynapse(bool)));
  setWindowSignalSlot(m_coarseBodyWindow);
  if(m_doc->getParentMvc() != NULL) {
    ZFlyEmMisc::Decorate3dBodyWindow(m_coarseBodyWindow, m_dvidInfo, m_doc->getParentMvc()->getView()->getViewParameter());
    ZFlyEmMisc::Decorate3dBodyWindowRoi(m_coarseBodyWindow, m_dvidInfo, getDvidTarget());
  }
}
void ZFlyEmProofMvc::makeBodyWindow() {
  ...
  connect(m_bodyWindow->getPunctaFilter(), SIGNAL(visibleChanged(bool)), doc, SLOT(showSynapse(bool)));
  setWindowSignalSlot(m_bodyWindow);
  if(m_doc->getParentMvc() != NULL) {
    ZFlyEmMisc::Decorate3dBodyWindow(m_bodyWindow, m_dvidInfo, m_doc->getParentMvc()->getView()->getViewParameter());
  }
}
void ZFlyEmProofMvc::makeSkeletonWindow() {
  ...
  connect(m_skeletonWindow->getPunctaFilter(), SIGNAL(visibleChanged(bool)), doc, SLOT(showSynapse(bool)));
  setWindowSignalSlot(m_skeletonWindow);
  if(m_doc->getParentMvc() != NULL) {
    ZFlyEmMisc::Decorate3dBodyWindow(m_skeletonWindow, m_dvidInfo, m_doc->getParentMvc()->getView()->getViewParameter());
  }
}
...
void ZFlyEmProofMvc::setDvidTarget(const ZDvidTarget& target) {
  ...
  if(getCompleteDocument() != NULL) {
    ...
    std::string startLog = "Start using UUID " + target.getUuid() + "@" + target.getAddressWithPort();
  ...
  emit messageGenerated(
    ZWidgetMessage(
      QString("Database %1 loaded.").arg(getDvidTarget().getSourceString(false).c_str()),
...
void ZFlyEmProofMvc::goToBody() {
  bool ok;
  QString text = QInputDialog::getText(this, tr("Go To"), tr("Body:"), QLineEdit::Normal, "", &ok);
  if(ok) {
    if(!text.isEmpty()) {
      ...
      if(bodyId > 0) {
        if(getSupervisor()->checkOut(bodyId)) {
          ...
          if(owner.empty()) {
            emit messageGenerated(
              ZWidgetMessage(
                QString("Failed to lock body %1. Is the librarian sever (%2) ready?").arg(bodyId).arg(getDvidTarget().getSupervisor().c_str()),
                NeuTube::MSG_ERROR));
          } else {
            emit messageGenerated(
              ZWidgetMessage(
                QString("Failed to lock body %1 because it has been locked by %2").arg(bodyId).arg(owner.c_str()), NeuTube::MSG_ERROR));
          }
        }
      }
    }
      ...
      if(checkOutBody(bodyId)) {
        ...
        if(getSupervisor() != NULL) {
          ...
          if(owner.empty()) {
            ...
            emit messageGenerated(
              ZWidgetMessage(
                QString("Failed to start annotation. %1 has been locked by %2").arg(bodyId).arg(owner.c_str()), NeuTube::MSG_ERROR));
          }
        }
      }
    } else {
...
void ZFlyEmProofMvc::launchSplit(uint64_t bodyId) {
  if(bodyId > 0) {
    if(!getCompleteDocument()->isSplittable(bodyId)) {
      ...
      emit messageGenerated(
        ZWidgetMessage(
          QString("Failed to launch split. %1 has been locked by %2").arg(bodyId).arg(owner.c_str()), NeuTube::MSG_ERROR));
    }
  } else {
...
void ZFlyEmProofMvc::locateBody(uint64_t bodyId, bool appending) {
  if(!getCompletePresenter()->isSplitWindow()) {
    ...
    if(reader.open(getDvidTarget())) {
      ...
      if(body.isEmpty()) {
        emit messageGenerated(
          ZWidgetMessage(QString("Cannot go to body: %1. No such body.").arg(bodyId), NeuTube::MSG_ERROR));
      } else {
        ZDvidInfo dvidInfo = reader.readGrayScaleInfo();
    ...
    if(m_coarseBodyWindow->hasRectRoi()) {
      ...
      if(reader.open(getDvidTarget())) {
        if(getCompletePresenter()->isSplitOn()) {
          ...
          if(body.isEmpty()) {
            emit messageGenerated(
              ZWidgetMessage(QString("Cannot crop body: %1. No such body.").arg(m_splitProject.getBodyId()),
                NeuTube::MSG_ERROR));
          } else {
            ZDvidInfo dvidInfo = reader.readGrayScaleInfo();
