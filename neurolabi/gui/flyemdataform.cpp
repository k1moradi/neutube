...
void FlyEmDataForm::viewModel(const QModelIndex& index) {
  ...
  QVector<const ZFlyEmNeuron*> neuronArray =
    m_neuronList->getNeuronArray(index);
#ifdef _DEBUG_2
  std::cout << m_neuronList->headerData(index.column(), Qt::Horizontal).toString().toStdString() << std::endl;
#endif
  ...
  } else if(m_neuronList->headerData(index.column(), Qt::Horizontal) == "Volume Path") {
...
ZStackDoc* FlyEmDataForm::showViewSelectedModel(ZFlyEmQueryView* view) {
  ...
  ZStackDoc* doc = new ZStackDoc;
  view->getModel()->retrieveModel(sel->selectedIndexes(), doc);
...
ZStackDoc* FlyEmDataForm::showViewSelectedBody(ZFlyEmQueryView* view) {
  ...
  ui->progressBar->show();
  QItemSelectionModel* sel = view->selectionModel();
  ZScalableStack* stack = view->getModel()->retrieveBody(sel->selectedIndexes());
  ...
  if(neuron != NULL) {
    if(!neuron->getThumbnailPath().empty()) {
      ...
      if(thumbnailReady) {
        ...
        int sourceZDim = getParentFrame()->getMasterData()->getSourceDimension(NeuTube::Z_AXIS);
        int sourceYDim = getParentFrame()->getMasterData()->getSourceDimension(NeuTube::Y_AXIS);
        ...
        if(reader.open(dvidTarget)) {
          ...
          if(sourceZDim == 0) {
            ...
            int z0 = getParentFrame()->getMasterData()->getSourceOffset(NeuTube::Z_AXIS);
            int y0 = getParentFrame()->getMasterData()->getSourceOffset(NeuTube::Y_AXIS);
...
void FlyEmDataForm::updateThumbnailLive(ZFlyEmNeuron* neuron) {
  ...
  QString threadId = QString("computeThumbnailFunc:%1").arg(neuron->getId());
  QList<QGraphicsItem*> itemList;
  if (m_bodyFutureMap.contains(threadId) &&
      m_bodyFutureMap[threadId].isRunning()) {
    // Thumbnail is computing asynchronously; nothing to do here.
    return;
  } else {
...
void FlyEmDataForm::saveVolumeRenderingFigure(
  ...
  if(neuron != NULL) {
    ...
    Z3DWindow* stage = new Z3DWindow(academy, Z3DWindow::INIT_FULL_RES_VOLUME, false, NULL);
