...
#endif
{
  ...
  m_softwareName = "3D Image Master";
#ifdef _QT_GUI_USED_
...
std::string NeutubeConfig::getPath(Config_Item item) const {
  ...
  case AUTO_SAVE:
#ifdef _QT_GUI_USED_
    return QDir(getPath(WORKING_DIR).c_str()).filePath("autosave").toStdString();
#else
...
NeutubeConfig::MainWindowConfig::MainWindowConfig()
  : m_tracingOn(true)
  , m_isMarkPunctaOn(true)
  , m_isSwcEditOn(true)
  , m_isExpandSwcWith3DWindow(false)
  , m_isProcessBinarizeOn(true)
  , m_isMaskToSwcOn(true)
  , m_isBinaryToSwcOn(true)
...
NeutubeConfig::Z3DWindowConfig::Z3DWindowConfig()
  : m_isUtilsOn(true)
  , m_isVolumeOn(true)
  , m_isGraphOn(true)
  , m_isSwcsOn(true)
...
NeutubeConfig::Z3DWindowConfig::SwcTabConfig::SwcTabConfig()
  : m_primitive("Normal")
  , m_colorMode("Branch Type")
  , m_zscale(1.0) {
}
