...
TEST(ZSwcMetric, measure)
{
  ...
  tree1.load((fs::path(GET_TEST_DATA_DIR)/"benchmark"/"bundle1"/"swc"/"1.swc").string().c_str());
  tree2.load((fs::path(GET_TEST_DATA_DIR)/"benchmark"/"bundle1"/"swc"/"2.swc").string().c_str());

  double dist = metric.measureDistance(&tree1, &tree2);
  EXPECT_DOUBLE_EQ(5000, dist);

  tree2.load((fs::path(GET_TEST_DATA_DIR)/"benchmark"/"bundle1"/"swc"/"3.swc").string().c_str());
  dist = metric.measureDistance(&tree1, &tree2);
  EXPECT_DOUBLE_EQ(19015.232570685483, dist);

  tree2.load((fs::path(GET_TEST_DATA_DIR)/"benchmark"/"bundle1"/"swc"/"4.swc").string().c_str());
  dist = metric.measureDistance(&tree1, &tree2);
  EXPECT_GT(0, dist);
