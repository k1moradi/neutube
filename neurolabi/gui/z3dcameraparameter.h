...
public:
  ...
  inline void setCamera(const glm::vec3& pos, const glm::vec3& focus) {
    m_value.setCamera(pos, focus, m_value.getUpVector());
    updatePara();
  }
  inline void setProjectionType(Z3DCamera::ProjectionType pt) { m_projectionType.select(pt == Z3DCamera::Perspective ? "Perspective" : "Orthographic"); }
  ...
  inline glm::vec3 worldToScreen(glm::vec3 wpt, glm::ivec4 viewport, Z3DEye eye = CenterEye) { return m_value.worldToScreen(wpt, viewport, eye); }
  inline glm::vec3 screenToWorld(glm::vec3 spt, glm::ivec4 viewport, Z3DEye eye = CenterEye) { return m_value.screenToWorld(spt, viewport, eye); }
