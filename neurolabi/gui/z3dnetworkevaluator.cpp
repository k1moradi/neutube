...
void Z3DNetworkEvaluator::buildNetwork() {
  ...
  while(!processQueue.empty()) {
    ...
    for(size_t i = 0; i < inports.size(); ++i) {
      ...
      for(size_t j = 0; j < connected.size(); ++j) {
        ...
        boost::add_edge(m_processorToVertexMapper[outProcessor], m_processorToVertexMapper[processor], EdgeInfo(connected[j], inports[i]), m_processorGraph);
      }
    }
  ...
  for(std::vector<Vertex>::reverse_iterator rit = sorted.rbegin(); rit != sorted.rend(); rit++) {
