 ...
 * Copyright (C) 2012-2013 Korea Institute of Science and Technology, Seoul.
...
bool Z3DInputPortBase::isConnectedTo(const Z3DOutputPortBase* port) const {
  return std::find(m_connectedOutputPorts.begin(), m_connectedOutputPorts.end(), port) != m_connectedOutputPorts.end();
}
...
bool Z3DOutputPortBase::isConnectedTo(const Z3DInputPortBase* port) const {
  return std::find(m_connectedInputPorts.begin(), m_connectedInputPorts.end(), port) != m_connectedInputPorts.end();
}
