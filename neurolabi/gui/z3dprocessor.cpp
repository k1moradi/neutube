...
void Z3DProcessor::deinitialize() {
  CHECK_GL_ERROR_NOTE("Z3DProcessor::deinitialize start");
  if(!isInitialized()) {
  ...
  CHECK_GL_ERROR_NOTE("Z3DProcessor::deinitialize end");
}
...
void Z3DProcessor::removePort(Z3DInputPortBase* port) {
  ...
  if(port->isInitialized()) {
    LWARN() << getClassName() << "port" << port->getName() << "has not been deinitialized";
  }
