...
int PointInPolygon(glm::dvec3 x, const std::vector<glm::dvec3>& pts, const double bounds[6], glm::dvec3 normal) {
  ...
  for(deltaVotes = 0, iterNumber = 1;
    (iterNumber < VTK_POLYGON_MAX_ITER) && (std::abs(deltaVotes) < VTK_POLYGON_VOTE_THRESHOLD); iterNumber++) {
    //  Generate ray
    bool rayOK;
...
bool TriangulatePolygon2(const std::vector<int64_t>& poly, const std::vector<glm::dvec3>& vertices, std::vector<size_t>& triangles) {
  ...
  // is convex and triangle removal can proceed without intersection checks.
  size_t numInQueue;
      ...
      if(ptId1 != qtId1 && ptId1 != qtId2 &&
        ...
        if((c1 ^ c2) || v1 * v1 < tol2 || v2 * v2 < tol2) {
          ...
          if(glm::dot(w, w) > 0) {
            ...
            int d1 = (u1 > 0);
            int d2 = (u2 > 0);
            if((d1 ^ d2)) { // One final check to make sure endpoints aren't coincident
              glm::dvec3 p = p1;
              glm::dvec3 q = q1;
              if(v2 * v2 < v1 * v1) { p = p2; }
              if(u2 * u2 < u1 * u1) { q = q2; }
