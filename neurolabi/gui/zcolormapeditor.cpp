  ...
  if(e->type() == QEvent::ToolTip) {
    ...
    if(findkey(helpEvent->pos(), index, true) != ZColorMapWidget::NONE) {
      ...
      if(m_colorMap->get().getKey(index).isSplit())
        tipText = QString("Key %1\nIntensity: %2\nLeft Color: %3\nRight Color: %4").arg(index + 1).arg(m_colorMap->get().getKey(index).getIntensity()).arg(m_colorMap->get().getKey(index).getQColorL().name()).arg(m_colorMap->get().getKey(index).getQColorR().name());
      else
        tipText = QString("Key %1\nIntensity: %2\nColor: %3").arg(index + 1).arg(m_colorMap->get().getKey(index).getIntensity()).arg(m_colorMap->get().getKey(index).getQColorL().name());
