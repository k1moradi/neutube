...
public: /* data operation */
  ...
  void clean();
  // Set stack data
  void setData(Stack* stack, C_Stack::Stack_Deallocator* delloc = C_Stack::kill);
...
class ZStack_Projection {
public:
  ZStack_Projection(Stack* parent = NULL)
    : m_parent(parent)
    , m_proj(NULL) {}
  ~ZStack_Projection() {
...
class ZStack_Stat {
public:
  ZStack_Stat(Stack* parent = NULL) {
    m_parent = parent;
    m_hist = NULL;
  }
  ~ZStack_Stat() {
    if(m_hist != NULL) { free(m_hist); }
  }
