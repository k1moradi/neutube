...
ZStack* ZStack::clone() const {
  ZStack* stack = NULL;
  if(hasData()) {
    stack = new ZStack(kind(), width(), height(), depth(), channelNumber());
...
void ZStack::logLSMInfo() {
  ...
  case 1:
    LINFO() << "DataType:" << "8-bit unsigned integer";
    break;
  case 2:
    LINFO() << "DataType:" << "12-bit unsigned integer";
    break;
  case 5:
    LINFO() << "DataType:" << "32-bit float(for \"Time Series Mean-of-ROIs\")";
    break;
  ...
  for(size_t i = 0; i < m_lsmInfo.m_lsmChannelDataTypes.size(); ++i) {
    ...
    case 1:
      LINFO() << "Channel" << i + 1 << "DataType:" << "8-bit unsigned integer";
      break;
    case 2:
      LINFO() << "Channel" << i + 1 << "DataType:" << "12-bit unsigned integer";
      break;
    case 5:
      LINFO() << "Channel" << i + 1 << "DataType:" << "32-bit float(for \"Time Series Mean-of-ROIs\")";
      break;
    }
  }
  ...
  case 0:
    LINFO() << "ScanType:" << "normal x-y-z-scan";
    break;
  case 1:
    LINFO() << "ScanType:" << "z-Scan (x-z-plane)";
    break;
  case 2:
    LINFO() << "ScanType:" << "line scan";
    break;
  case 3:
    LINFO() << "ScanType:" << "time series x-y";
    break;
  case 4:
    LINFO() << "ScanType:" << "time series x-z (release 2.0 or later)";
    break;
  case 5:
    LINFO() << "ScanType:" << "time series \"Mean of ROIs\" (release 2.0 or later)";
    break;
  case 6:
    LINFO() << "ScanType:" << "time series x-y-z (release 2.3 or later)";
    break;
  case 7:
    LINFO() << "ScanType:" << "spline scan (release 2.5 or later)";
    break;
  case 8:
    LINFO() << "ScanType:" << "spline plane x-z (release 2.5 or later)";
    break;
  case 9:
    LINFO() << "ScanType:" << "time series spline plane x-z (release 2.5 or later)";
    break;
  case 10:
    LINFO() << "ScanType:" << "point mode (release 3.0 or later)";
    break;
  }
  switch(m_lsmInfo.m_basicInfo.u16SpectralScan) {
  case 0:
    LINFO() << "SpectralScan:" << "no spectral scan";
    break;
  case 1:
    LINFO() << "SpectralScan:" << "image has been acquired in spectral scan mode with a META detector (release 3.0 or later)";
    break;
  }
  switch(m_lsmInfo.m_basicInfo.u32DataType) {
  case 0:
    LINFO() << "DataType:" << "Original scan data";
    break;
  case 1:
    LINFO() << "DataType:" << "Calculated data";
    break;
  case 2:
    LINFO() << "DataType:" << "Animation";
    break;
  }
...
bool ZStack::paste(ZStack* dst, int valueIgnored, double alpha) const {
  if(dst != NULL) {
    ...
    int z0 = offset.getZ();
    for(int i = 0; i < ch; ++i) {
      C_Stack::setBlockValue(dst->c_stack(i), c_stack(i), x0, y0, z0, valueIgnored, -1, alpha);
    }
    return true;
  }
  return false;
