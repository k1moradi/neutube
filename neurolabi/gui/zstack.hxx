 ...
 *Therefore, voxels from the same row are stored contiguously in an array. The
...
public: /* processing routines */
   ...
   *
   * The offset position is adjusted accordingly.
   */
...
public:
  ...
  std::vector<ZVec3Parameter*>& channelColors() {
    initChannelColors();
    return m_channelColors;
  }
  glm::vec3 getChannelColor(size_t ch) {
    initChannelColors();
    return m_channelColors[ch]->get();
  }
