...
ZUndoCommand::ZUndoCommand(QUndoCommand* parent)
  : QUndoCommand(parent)
  , INIT_ZUNDOCOMMAND {
}
...
void ZUndoCommand::logCommand(const QString& msg) const {
  ...
  // TODO: implement or route to logger
  Q_UNUSED(msg);
#ifdef _DEBUG_
...
void ZStackDocCommand::SwcEdit::ChangeSwcCommand::recover() {
  ...
  for(std::map<Swc_Tree_Node*, Swc_Tree_Node>::iterator
        iter = m_backupSet.begin();
    iter != m_backupSet.end(); ++iter) {
...
ZStackDocCommand::SwcEdit::TranslateRoot::TranslateRoot(
  ...
  , m_y(y)
  , m_z(z) {
  setText(QObject::tr("translate swc tree root to (%1,%2,%3)").arg(m_x).arg(m_y).arg(m_z));
}
ZStackDocCommand::SwcEdit::TranslateRoot::~TranslateRoot() {
...
ZStackDocCommand::SwcEdit::Rescale::Rescale(
  ...
  setText(QObject::tr("rescale swc tree (%1,%2,%3)").arg(m_scaleX).arg(m_scaleY).arg(m_scaleZ));
}
ZStackDocCommand::SwcEdit::Rescale::~Rescale() {
  for(int i = 0; i < m_swcList.size(); i++) {
    delete m_swcList[i];
...
ZStackDocCommand::SwcEdit::RescaleRadius::RescaleRadius(
  ...
  , m_enddepth(enddepth) {
  if(enddepth < 0) {
    setText(QObject::tr("rescale radius of swc nodes with depth in [%1,max) by %2").arg(startdepth).arg(scale));
  } else {
    setText(QObject::tr("rescale radius of swc nodes with depth in [%1,%2) by %3").arg(startdepth).arg(enddepth).arg(scale));
  }
}
ZStackDocCommand::SwcEdit::RescaleRadius::~RescaleRadius() {
  for(int i = 0; i < m_swcList.size(); i++) {
...
ZStackDocCommand::SwcEdit::SetParent::SetParent(ZStackDoc* doc, Swc_Tree_Node* node, Swc_Tree_Node* parentNode,
  ...
  , m_doc(doc)
  , m_node(node)
  , m_newParent(parentNode)
  , m_oldParent(NULL)
  , m_prevSibling(NULL)
  , m_deletingOrphan(deletingOrphan)
  , m_isExecuted(false) {
...
ZStackDocCommand::TubeEdit::Trace::Trace(
  ...
  , m_z(z)
  , m_c(c) {
  setText(QObject::tr("trace tube from (%1,%2,%3) in channel %4").arg(x).arg(y).arg(z).arg(c));
}
