...
public:
  ...
  enum EUpdateOption { UPDATE_NONE, UPDATE_QUEUED, UPDATE_DIRECT };
  /*!
  ...
  inline void setSizeHintOption(NeuTube::ESizeHintOption option) { m_sizeHintOption = option; }
  inline void blockRedraw(bool state) { m_isRedrawBlocked = state; }
public:
...
public slots:
  ...
  void paintMask();
  void paintObject();
  void paintObject(QList<ZStackObject *> selected, QList<ZStackObject *> deselected);
...
protected:
  ...
  void clearCanvas();
  template<typename T> void resetCanvasWithStack(T &canvas, ZPainter *painter);
  virtual void resetCanvasWithStack(ZMultiscalePixmap &canvas, ZPainter *painter);
