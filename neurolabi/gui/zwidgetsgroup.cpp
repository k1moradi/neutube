...
void ZWidgetsGroup::setVisible(bool visible) {
  ...
  m_isVisible = visible;
  // Propagate to children based on type
  switch(m_type) {
  case PARAMETER:
    if(m_parameter) m_parameter->setVisible(visible);
    break;
  case WIDGET:
    if(m_widget) m_widget->setVisible(visible);
    break;
  case GROUP:
    for(int i = 0; i < m_childGroups.size(); i++) {
      m_childGroups[i]->setVisible(visible);
    }
    break;
  }
}
