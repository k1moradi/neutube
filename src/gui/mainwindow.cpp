...
#ifdef _DEBUG_2
  std::cout << NeutubeConfig::getInstance().getPath(NeutubeConfig::SETTINGS) << std::endl;
#endif
...
{
  QString fileName = getOpenFileName("Open stack","Stack files (*.tif *.lsm *.raw *.png *.swc *.nnt *.apo *.marker *.json)");
#if 0
  QFileDialog::getOpenFileName(this, tr("Open stack"),m_lastOpenedFilePath,tr("Stack files (*.tif *.lsm *.raw *.png *.swc *.nnt *.apo *.marker *.json)"),NULL);
#endif
  if (!fileName.isEmpty()) {
...
void MainWindow::on_actionSave_triggered()
{
  ...
  if (frame != NULL) {
    if (frame->isReadyToSave()) {
      ...
      qDebug() << dataFile;
      QString fileName = getSaveFileName("Save tracing project", "Tracing project",dataFile + "/untitled.trace");
#if 0
      QFileDialog::getSaveFileName(this, tr("Save tracing project"), dataFile + "/untitled.trace", tr("Tracing project"), 0);
#endif
      qDebug() << fileName;
      if (!fileName.isEmpty()) {
...
void MainWindow::on_actionAdd_Reference_triggered()
{
  ZStackFrame *frame = currentStackFrame();
  if (frame != NULL) {
    QStringList pathList = getOpenFileNames("Add SWC Reference", "Swc files (*.swc)");
#if 0
    QFileDialog::getOpenFileNames(this, tr("Add SWC Reference"), m_lastOpenedFilePath, "Swc files (*.swc)", NULL);
#endif
    if (!pathList.isEmpty()) {
...
#if 0
      QFileDialog::getOpenFileName(this, tr("Open Project"), m_lastOpenedFilePath, tr("XML files (*.xml)"));
#endif
  m_lastOpenedFilePath = fileName;
  ...
  if (frame != NULL) {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      if (frame->document()->hasStackData()) {
        ...
        proc.distanceTransform(stack, dlg.isSquared(), dlg.isSliceWise());
...
void MainWindow::on_actionExpand_Region_triggered()
{
  if (currentStackFrame() != NULL) {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      if (currentStackFrame()->document()->getStack()->isBinary()) {
        proc.expandRegion(currentStackFrame()->document()->getStack(), dlg.getRadius());
        currentStackFrame()->document()->notifyStackModified();
        currentStackFrame()->updateView();
      }
    }
...
#if 0
  QFileDialog::getOpenFileName(this, tr("Open Network"), m_lastOpenedFilePath, tr("FlyEM network files (*.txt)"));
#endif
  if (!fileName.isEmpty()) {
    m_lastOpenedFilePath = fileName;
...
#if 0
    QFileDialog::getOpenFileNames(this, tr("Load SWC files"), m_lastOpenedFilePath, tr("SWC files (*.swc)"), NULL);
#endif
    if (!fileList.isEmpty()) {
      frame->load(fileList);
...
#if 0
    QFileDialog::getOpenFileName(this, tr("Open Network"), m_lastOpenedFilePath, tr("FlyEM network files (*.fnt)"));
#endif
  if (!fileName.isEmpty()) {
    m_lastOpenedFilePath = fileName;
    getProgressDialog()->setRange(0, 2);
...
#if 0
      QFileDialog::getOpenFileName(this, tr("Import mask"), m_lastOpenedFilePath, tr("Image file (*.tif *.xml *.json)"));
#endif
  if (!fileName.isEmpty()) {
    ZStackFrame *frame = currentStackFrame();
    if (frame != NULL) {
...
#if 0
      QFileDialog::getOpenFileName(this, tr("Import FlyEM segmentation"), m_lastOpenedFilePath, tr("Segmentation file (*.json)"));
#endif
  if (!fileName.isEmpty()) {
  ...
  if (!fileName.isEmpty()) {
    ...
    if (doc->importAxonExport(fileName.toStdString())) {
      ...
      QMessageBox::critical(this, tr("Error"),
                            QString("<font size=4 face=Times>I cannot open ") + "<i>" + fileName + "</i>" + "</font>",
                            QMessageBox::Ok);
    }
  }
}
...
void MainWindow::reportFileOpenProblem(const QString &filePath, const QString &reason)
{
...
#if 0
  QMessageBox::critical(this, tr("Error"),
                      QString("<font size=4 face=Times>I cannot open ") + "<i>" + filePath + "</i>: " + finalReason + "</font>",
                      QMessageBox::Ok);
#endif
}
...
void MainWindow::on_actionMask_triggered()
{
  ...
  if (frame != NULL) {
    ...
    QString fileName = getSaveFileName("Save Mask", "TIF files (*.tif)",
                                       path.c_str());
#if 0
    QFileDialog::getSaveFileName(this, tr("Save Mask"), path.c_str(), tr("TIF files (*.tif)"), NULL);
#endif
    if (!fileName.isEmpty()) {
...
void MainWindow::on_actionMask_SWC_triggered()
{
  if (m_skeletonDlg->exec() == QDialog::Accepted) {
    ...
    if (frame != NULL) {
      ...
      if (maskArray.empty()) {
        report("Skeletonization Failed", "No mask found. No SWC generated", NeuTube::MSG_WARNING);
        return;
      }
      ...
      if (wholeTree != NULL) {
        ...
        if (stackFrame != swcFrame) {
          swcFrame->document()->executeAddSwcBranchCommand(wholeTree, distThre);
        } else {
          ...
          if (frame == stackFrame) {
            ...
            new ZStackDocCommand::SwcEdit::AddSwc(frame->document().get(), proj, maskCommand);
#endif
            frame->document()->pushUndoCommand(maskCommand);
          }
          stackFrame->document()->pushUndoCommand(command);
          stackFrame->document()->executeAddSwcBranchCommand(wholeTree, distThre);
        }
        if (frame != stackFrame) {
        ...
        progressDlg->reset();
        report("Skeletonization failed", "No SWC tree generated.", NeuTube::MSG_ERROR);
      }
    }
  }
}
...
#if 0
        QFileDialog::getOpenFileNames(this, tr("Load files"), m_lastOpenedFilePath, tr("SWC/Image files (*.swc *.png *.tif)"), NULL);
#endif
...
void MainWindow::on_actionSave_SWC_triggered()
{
  ...
  if (frame != NULL) {
    if (frame->document()->hasSwc()) {
      ...
      QString fileName = getSaveFileName("Save neuron as SWC", tr("SWC file (*.swc)"), swcSource.c_str());
#if 0
          QFileDialog::getSaveFileName(this, tr("Save neuron as SWC"), swcSource.c_str(), tr("SWC file (*.swc)"));
#endif
      if (!fileName.isEmpty()) {
...
void MainWindow::createDvidFrame()
{
  ...
  if (reader.hasData()) {
    ...
    report("No data retrieved", "No data retrieved from DVID", NeuTube::MSG_WARNING);
  }
  dvidBuffer->clear();
  progressDlg->reset();
#if 0
  ...
  if (!obj.isEmpty()) {
    ...
    if (m_dvidObjectDlg->retrievingSkeleton()) {
      m_dvidClient->postRequest(ZDvidClient::DVID_GET_SWC, QVariant(m_dvidObjectDlg->getBodyId()));
#if 0
      ...
      wholeTree->save(QString("%1/%2.swc").arg(m_dvidClient->getTmpDirectory()).arg(m_dvidObjectDlg->getBodyId()).toStdString());
      m_dvidClient->postRequest(ZDvidClient::DVID_UPLOAD_SWC, QVariant(m_dvidObjectDlg->getBodyId()));
#endif
    }
    m_dvidFrame = NULL;
  }
...
#if 0
    ZStackFrame *frame = hotSpotDemo(bodyId, m_dvidObjectDlg->getAddress(), dataInfo.getDvidUuid());
#else
